...
See `.gitignore` for legacy file patterns that are automatically excluded.
...
Iron Manus MCP includes comprehensive security protections:

- **SSRF Protection**: Blocks private/localhost IPs (192.168.x.x, 127.x.x.x, etc.)
- **URL Validation**: Validates URL schemes (HTTP/HTTPS only)
- **Allowlist Enforcement**: Enforces allowed hosts when configured
- **Request Limits**: Timeout and size limits on requests
- **Rate Limiting**: Per-API-endpoint rate limiting
