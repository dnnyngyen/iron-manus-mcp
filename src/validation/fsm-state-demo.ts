...
async function runValidationDemo(): Promise<void> {
  ...
  validator.logPhaseTransition(
    ...
    {
      current_todos: [
        { id: 'task-1', content: 'Implement JWT', status: 'pending', priority: 'high' },
      ],
    },
    150
  );
  ...
  if (transitionLog) {
    ...
    console.log(
      `Success Rate: ${((transitionLog.successfulTransitions / transitionLog.totalTransitions) * 100).toFixed(1)}%`
    );
  ...
  for (let i = 0; i < demoScenarios.length; i++) {
    ...
    console.log(
      `✓ State Consistency Score: ${(result.metrics.stateConsistencyScore * 100).toFixed(1)}%`
    );
    console.log(
      `✓ Transition Integrity Score: ${(result.metrics.transitionIntegrityScore * 100).toFixed(1)}%`
    );
    console.log();
    ...
    if (result.errors.length > 0) {
      ...
      result.errors.forEach(error => {
        const severityIcon =
          error.severity === 'critical' ? '🚨' : error.severity === 'high' ? '⚠️' : '⚡';
        console.log(
          `  ${severityIcon} [${error.severity.toUpperCase()}] ${error.code}: ${error.message}`
        );
        if (error.context) {
          console.log(
...
function generateValidationReport(validator: FSMStateValidator, scenarios: any[]): void {
  ...
  console.log(
    `Valid Scenarios: ${validScenarios} (${((validScenarios / totalScenarios) * 100).toFixed(1)}%)`
  );
  ...
  return {
    ...
    payload: {
      ...
      api_usage_metrics: {
        ...
        knowledge_synthesis_quality: variant === 'healthy' ? 0.88 : 0.7,
      },
    },
      ...
      current_todos: [
        {
          id: 'ui-1',
          content:
            '(ROLE: ui_architect) (CONTEXT: dashboard) Design responsive layout (OUTPUT: layout_specs)',
          status: 'completed',
          priority: 'high',
          type: 'TaskAgent',
          meta_prompt: {
        ...
        },
        {
          id: 'ui-2',
          content:
            '(ROLE: ui_implementer) (CONTEXT: charts) Implement D3.js charts (OUTPUT: chart_components)',
          status: 'in_progress',
          priority: 'high',
          type: 'TaskAgent',
          meta_prompt: {
