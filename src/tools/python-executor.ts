...
/**
 * Python code execution arguments - validated with Zod schema
 * @see PythonExecutorArgsSchema in validation/schemas.ts
 */
...
export class PythonExecutorTool extends BaseTool {
  ...
  readonly description =
    'Computational thinking catalyst - transforms abstract problems into executable logic. When facing complex analysis, ask: How do I break this into algorithmic steps? What computational patterns will reveal insights hidden in manual approaches? This tool guides you to think like a computer scientist: systematic, iterative, and data-driven. Use it to challenge yourself: Am I leveraging computational power to discover what human intuition alone cannot see?';
  ...
  readonly inputSchema: ToolSchema = {
    ...
    properties: {
      code: {
        type: 'string',
        description:
          'Computational logic expression - How does your abstract thinking translate into precise, executable steps? What algorithmic approach captures your analytical intent?',
      },
      setup_libraries: {
        type: 'array',
        items: { type: 'string' },
        description:
          'Cognitive tool dependencies - Which computational instruments will extend your analytical capabilities beyond what manual reasoning can achieve?',
      },
      description: {
        type: 'string',
        description:
          'Intent articulation - Why does this computation matter? What insights or patterns are you trying to uncover through algorithmic exploration?',
      },
    },
    required: ['code'],
   ...
   * ```
   */
  protected validateArgs(args: unknown): asserts args is PythonExecutorArgs {
    const result = PythonExecutorArgsSchema.safeParse(args);
    if (!result.success) {
  ...
  async handle(args: unknown): Promise<ToolResult> {
    try {
      ...
      // Now TypeScript knows args is PythonExecutorArgs
  ...
  private prepareCode(args: PythonExecutorArgs): string {
    ...
    // Security: Validate Python code before preparation
  ...
  // Security validation now handled by shared utility in src/utils/python-security.ts

  // Library validation now handled by shared utility in src/utils/python-security.ts
  ...
  private generateLibrarySetup(libraries: string[]): string {
    // Security: Validate library names using shared utility
...
export class EnhancedPythonDataScienceTool extends BaseTool {
  ...
  readonly description =
    "Data science thinking orchestrator - guides you through the analytical journey from raw data to insights. When confronting data challenges, ask: What story is this data trying to tell? How do I uncover patterns that reveal deeper truths? This tool doesn't just execute analysis - it prompts you to think like a data scientist: hypothesis-driven, methodical, and insight-seeking. Challenge yourself: What assumptions about this data might I be missing? What angles haven't I explored?";
  ...
  readonly inputSchema: ToolSchema = {
    ...
    properties: {
      operation: {
        ...
        description:
          'Analytical approach - What type of data science thinking best matches your investigative intent? Each operation embodies a different way of interrogating information.',
      },
      input_data: {
        type: 'string',
        description:
          'Information substrate - What raw material will fuel your analytical exploration? Consider the nature and potential stories within this data.',
      },
      parameters: {
        type: 'object',
        description:
          'Investigation constraints - What specific angles or approaches will focus your analytical attention and guide deeper discovery?',
      },
      custom_code: {
        type: 'string',
        description:
          'Unique analytical logic - How does your specific domain knowledge translate into computational steps that standard operations cannot capture?',
      },
    },
    required: ['operation'],
    additionalProperties: false,
   ...
   * ```
   */
  protected validateArgs(args: unknown): asserts args is EnhancedPythonDataScienceArgs {
    const result = EnhancedPythonDataScienceArgsSchema.safeParse(args);
    if (!result.success) {
      const errors = result.error.errors.map(e => `${e.path.join('.')}: ${e.message}`).join(', ');
      throw new Error(`Invalid arguments: ${errors}`);
    }
  ...
  async handle(args: unknown): Promise<ToolResult> {
    try {
      this.validateArgs(args);
      // Now TypeScript knows args is EnhancedPythonDataScienceArgs
  ...
  private generateOperationCode(
    ...
    if (operation === 'custom' && customCode) {
      // Security: Validate custom Python code before execution
      validatePythonCode(customCode);
   ...
   * ```
   */
  private generateWebScrapingCode(url?: string, _params?: Record<string, unknown>): string {
    // Security: Validate URL before generating scraping code
    if (url) {
      try {
        validateAndSanitizeURL(url);
      } catch (error) {
        throw new Error(
          `SSRF Protection: Invalid URL provided - ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
    ...
    return this.wrapWithLibrarySetup(
      `
${this.generateWebScrapingImports()}
${this.generateWebScrapingSetup(url)}
${this.generateWebScrapingExtraction()}
${this.generateWebScrapingOutput()}
`,
      ['requests', 'bs4', 'pandas']
    );
  }
  ...
  /**
   * Generates import statements for web scraping
   */
  private generateWebScrapingImports(): string {
    return `# Web scraping with BeautifulSoup and requests
import requests
...
import pandas as pd`;
  ...
  /**
   * Generates web scraping setup code
   */
  private generateWebScrapingSetup(url?: string): string {
    return `
...
headers = {'User-Agent': 'Mozilla/5.0 (compatible; Data Science Tool)'}`;
  ...
  /**
   * Generates web scraping data extraction code
   */
  private generateWebScrapingExtraction(): string {
    return `
try:
  ...
  /**
   * Generates web scraping output and results code
   */
  private generateWebScrapingOutput(): string {
    return `    
   ...
   * ```
   */
  private generateDataAnalysisCode(data?: string, _params?: Record<string, unknown>): string {
    return this.wrapWithLibrarySetup(
      `
${this.generateDataAnalysisImports()}
${this.generateDataAnalysisDataSetup(data)}
${this.generateDataAnalysisOverview()}
${this.generateDataAnalysisStatistics()}
${this.generateDataAnalysisQuality()}
${this.generateDataAnalysisCorrelation()}
${this.generateDataAnalysisCompletion()}
`,
      ['pandas', 'numpy']
    );
  }
  ...
  /**
   * Generates import statements for data analysis
   */
  private generateDataAnalysisImports(): string {
    return `# Data analysis with pandas and numpy
import pandas as pd
import numpy as np
from io import StringIO`;
  ...
  /**
   * Generates data setup code for analysis (handles provided data or creates sample)
   */
  private generateDataAnalysisDataSetup(data?: string): string {
    return `
...
`
  ...
  /**
   * Generates dataset overview analysis code
   */
  private generateDataAnalysisOverview(): string {
    return `
...
print(f"Memory usage: {df.memory_usage(deep=True).sum() / 1024:.2f} KB")`;
  ...
  /**
   * Generates statistical summary analysis code
   */
  private generateDataAnalysisStatistics(): string {
    return `
print("\\n📈 STATISTICAL SUMMARY")
print("=" * 50)
print(df.describe())`;
  ...
  /**
   * Generates data quality check analysis code
   */
  private generateDataAnalysisQuality(): string {
    return `
print("\\n🔍 DATA QUALITY CHECK")
print("=" * 50)
...
print(df.dtypes)`;
  ...
  /**
   * Generates correlation analysis code
   */
  private generateDataAnalysisCorrelation(): string {
    return `
...
if len(numeric_cols) > 1:
  ...
  /**
   * Generates analysis completion message
   */
  private generateDataAnalysisCompletion(): string {
    return `
print("\\n✅ Data analysis completed successfully")`;
   ...
   * ```
   */
  private generateVisualizationCode(data?: string, _params?: Record<string, unknown>): string {
    return this.wrapWithLibrarySetup(
      `
...
`,
   ...
   * ```
   */
  private generateMLCode(data?: string, _params?: Record<string, unknown>): string {
    return this.wrapWithLibrarySetup(
      `
